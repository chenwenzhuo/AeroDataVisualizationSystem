var popup = new function() {
	...
	this.open = function(width, height, title, url, closed) {
		...
		var html = '<div class="win"><div class="mask-layer"></div><div class="window-panel"><iframe class="title-panel" frameborder="0" marginheight="0" marginwidth="0" scrolling="no"></iframe><div class="title"><h3></h3></div><a href="javascript:void(0)" onclick="popup._close();" class="close-btn" title="关闭">×</a><iframe class="body-panel" frameborder="0" marginheight="0" marginwidth="0" scrolling="auto" src=""></iframe></div></div>';
		var jq = $(html);
		jq.find(".window-panel").height(popup.height).width(popup.width).css("margin-left", -popup.width / 2).css("margin-top", -popup.height);
	...
	function messageBox(html, title, message, type) {
		var jq = $(html);
		if(type === "toast") {
			jq.find(".window-panel").width(message.length * 20).css("margin-left", -message.length * 20 / 2).css("margin-top", -popup.height / 2);
		} else {
			jq.find(".window-panel").width(popup.width).css("margin-left", -popup.width / 2).css("margin-top", -popup.height / 2 - 36);
		}
		if(valempty(title)) {
...
function valempty(str) {
	return str === "null" || str == null || str === "" || str === "undefined" || str === undefined || str === 0;
}